...
public class TeacherDao extends BaseDao {
	...
	public List<Texam> queryexam() throws SQLException {
		...
		String sql = "select * from texam t,tpaperinfo tp  where t.ischeck=0 and t.pid = tp.pid ";
		openConnection();
		PreparedStatement ps = conn.prepareStatement(sql);
	...
	public void addscore(String sno, int pid, int score) throws SQLException {
		String sql = "update tstuansinfo set score=?,	state=3 where sno=? and examid=?";
		openConnection();
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setInt(1, score);
		ps.setString(2, sno);
	...
	public List<Tpaperinfo> querypaper() throws SQLException {
		...
		String sql = "select * from tpaperinfo tp,texam t  where t.pid=tp.pid and t.ischeck=0";
		openConnection();
		PreparedStatement ps = conn.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public List<Tpaperinfo> querypaperinfo(String ptitle) throws SQLException {
		String sql = "select * from tpaperinfo,tcourse where ptitle like '%" + ptitle
				+ "%'  and  tpaperinfo.cno=tcourse.cno";
		openConnection();
		PreparedStatement ps = conn.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		List<Tpaperinfo> paperinfo = new ArrayList<Tpaperinfo>();
